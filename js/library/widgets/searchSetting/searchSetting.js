...
define([
...
], function (Dictionary,SearchConfig,Search,FeatureLayer,arcgisUtils,declare, domConstruct, domStyle, domAttr, lang, on, win, domGeom, dom, array, domClass, Query, Deferred, QueryTask, Point, template, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, sharedNls, topic, urlUtils, ActivitySearch, esriRequest, EventPlannerHelper, LocatorTool, a11yclick, locale) {
    // ========================================================================================================================//
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, ActivitySearch, EventPlannerHelper], {
        ...
        */
        postCreate: function () {
            ...
            */
            topic.subscribe("toggleWidget", lang.hitch(this, function (widget) {
                if (widget !== "searchSetting") {
                    ...
                    if (domClass.contains(this.divSearchContainer, "esriCTHideContainerHeight")) {
                        ...
                        domStyle.set(this.divSearchContainer, "height", "71%");
                    }
                }
            }));
            ...
            this.own(on(this.domNode, a11yclick, lang.hitch(this, function () {
                //李骁 Add Search Widget 模糊查询
                if(this.searchFuzzily != null){
                    this.searchFuzzily.destroy();//连div都清空了。 应该动态添加。
                //<div id="searchFuzzily" ></div> 最开始的静态的div 其实也可以动态生成。 都行。
                //    this.searchFuzzilyDiv = domConstruct.create("div", { "id": "searchFuzzily", "class": null }, null);
                //    this.searchFuzzilyDiv = domConstruct.create("div", { "id": "searchFuzzily", "class": null }, this.divSearchContent,first);//first必须是字符串
                    this.searchFuzzilyDiv = domConstruct.create("div", { "id": "searchFuzzily", "class": null }, this.divSearchContent,"first");//ok 成功！first必须是字符串
                }
            ...
            dojo.queryLayerUrl = this.selectlayer.value;
            dojo.fieldNameQ = this.selectfieldName.selectedOptions[0].label;
            dojo.fieldMethodQ = this.selectfieldMethod.selectedOptions[0].label;
            // 李骁  选择图层 查询字段方法、数值  联动  start
            this.own(on(this.selectlayer, "change", lang.hitch(this, function () {
                url =  this.selectlayer.value;
                dojo.queryLayerUrl = this.selectlayer.value;
                this._updateFieldName(url);
            })));
            //selectfieldName  查询字段的 change事件 监听
            this.own(on(this.selectfieldName, "change", lang.hitch(this, function () {
                var fieldName =  this.selectfieldName.value;
                dojo.fieldNameQ = this.selectfieldName.selectedOptions[0].label;
                this._updateFieldvalue(fieldName);
            })));
            //selectfieldValue  查询方法的 change事件 监听
            this.own(on(this.selectfieldMethod, "change", lang.hitch(this, function () {
                var fieldMethod =  this.selectfieldMethod.value;
                dojo.fieldMethodQ = this.selectfieldMethod.selectedOptions[0].label;
            })));
            //selectfieldValue  查询数值的 change事件 监听
            this.own(on(this.selectfieldValue, "change", lang.hitch(this, function () {
                var selectfieldValueOption = this.selectfieldValue.selectedOptions[0];
                //dojo.fieldValueQ = selectfieldValueOption.label;//label 有省略的就查询不出来了
                dojo.fieldValueQ = selectfieldValueOption.title;//这个名称完整
            })));
            // 李骁  选择图层 查询字段、方法、数值  联动  end
        ...
        //李骁  start
        //Add Search Widget 模糊查询
        AddSearchWidget:function(){
            this.searchFuzzily = new Search({
            ...
            var sources = this.searchFuzzily.get("sources");

            for (var i=0;i<dojoSearchConfig.featureServers.length;i++) {
                var SearchFields = dojoSearchConfig.featureServers[i].searchFields;
                var DisplayField = dojoSearchConfig.featureServers[i].displayField;
                var Featurelayer = dojoSearchConfig.featureServers[i].url;
                var Name = dojoSearchConfig.featureServers[i].layerName;
                sources.push({
            ...
            this.searchFuzzily.set("sources", sources);
            this.searchFuzzily.startup();

            on(this.searchFuzzily,'search-results', function(e) {
                var e = e;
                var a = 1;
            });
        },
        ...
        /**
         * Get layer from map  李骁   这个地方以后需要修改
         */
        _getLayerName:function(){
            // 清空，下拉框保留第一个选项。 20160105  start
            document.getElementById("selectLayer").options.length = 1;
            document.getElementById("fieldName").options.length = 1;
            document.getElementById("fieldValue").options.length = 1;
            // 清空，下拉框保留第一个选项。 20160105  end
            //获得图层信息的代码 start
            var layers = arcgisUtils.getLayerList(dojo.mapResponse);
            var layerIndex = appGlobals.configData.LocatorSettings.QueryLayerIndex;
            dojo.queryLayerDetail = layers[layerIndex];
            var layersUrls = layers[layerIndex].layer.url;
            dojo.queryLayerUrl = layersUrls;
            ...
            for (var i=0;i<layers.length;i++) {
                ...
                layerurl = layers[i].layer._url.path;
                document.getElementById("selectLayer").options.add(new Option(layername,layerurl));
            }
            //获得图层信息的代码 end
        },
        _updateFieldName:function(url){
            var layerURL = url;
            ...
            query.outFields = ["*"];
            query.where = "1=1";
            if(layerURL != "pleaseSelectLayer") {
                queryTask.execute(query, this._getFieldName);
            }
        },
        //updateFieldvalue
        _updateFieldvalue:function(FieldName){
            fieldName = FieldName;
            ...
            query.outFields = ["*"];
            query.where = "1=1";
            queryTask.execute(query, this._getFieldValue);
        },

        _getFieldValue:function(results){
            //dojo.fieldValueQ = results.features[0].attributes[fieldName];//这个先不需要了。在change事件中 获得 查询的参数值
            var resultCount = results.features.length;
            //document.getElementById("fieldValue").options.length=0;
            document.getElementById("fieldValue").options.length=1;
            keyPairs = new HashMap();
            var j = 0;
            var defaultOptionLength = appGlobals.configData.LayersConfigInfo.DefaultOptionLength;
            for (var i = 0; i < resultCount; i++) {
                var fieldValue =results.features[i].attributes[fieldName];
                var fieldValueNew;
                if(fieldValue.length>defaultOptionLength){
                    fieldValueNew  = fieldValue.substr(0,20)+"...";
                }else{
                    fieldValueNew = fieldValue;
                }
                //if(!keyPairs.containsValue(fieldValue)) {
                if(!keyPairs.containsValue(fieldValueNew)) {
                    //var option = new Option(fieldValue,fieldValue);
                    var option = new Option(fieldValueNew,fieldValueNew);
                    option.title = fieldValue;
                    document.getElementById("fieldValue").options.add(option);
                    keyPairs.put(j,fieldValueNew);
                    //keyPairs.put(j,fieldValue);
                    j++;
                }
            }
        },

        _getFieldName:function(results){
            var resultCount = results.fields.length;
            dojo.fieldsFromLayer = results.fields;
            //document.getElementById("fieldName").options.length=0;
            document.getElementById("fieldName").options.length=1;
            for (var i = 0; i < resultCount; i++) {
                var fieldName = results.fields[i].name;
                //var fieldAlias = results.fields[i].alias;//后面会用到   label 不能用alias 还是应该用name
                var fieldAlias = results.fields[i].name;
                var option = new Option(fieldName,fieldAlias);
                option.title = fieldAlias;
                //document.getElementById("fieldName").options.add(new Option(fieldName,fieldAlias));
                document.getElementById("fieldName").options.add(option);
            }
            //然后默认查询第一个字段的 所有数值。
            var fieldName =  results.fields[0].name;
            this._updateFieldvalue(fieldName);
        },
        //李骁  end
        /**
