...
define([
    ...
    "dijit/a11yclick",
    "../myList/myListHelper"
//], function (jquery,exporting,HighChartsJS,Query,QueryTask,Dictionary,ChartConfig,arcgisUtils,declare, domConstruct, lang, on, dom, domAttr, domClass, html, domStyle, locale, template, win, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, sharedNls, topic, date, Point, array, string, query, a11yclick, MyListHelper) {
//], function (jquery,HighChartsJS,Query,QueryTask,Dictionary,ChartConfig,arcgisUtils,declare, domConstruct, lang, on, dom, domAttr, domClass, html, domStyle, locale, template, win, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, sharedNls, topic, date, Point, array, string, query, a11yclick, MyListHelper) {
], function (HighChartsJS,Query,QueryTask,Dictionary,ChartConfig,arcgisUtils,declare, domConstruct, lang, on, dom, domAttr, domClass, html, domStyle, locale, template, win, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, sharedNls, topic, date, Point, array, string, query, a11yclick, MyListHelper) {
    //========================================================================================================================//
    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, MyListHelper], {
        ...
        url:null,
        countByCategoryFieldName:null,
        CategotykeyPairs:null,
        /**
        ...
        */
        postCreate: function () {
            ...
            */
            ...
            this.domNode = domConstruct.create("div", { "title": sharedNls.tooltips.chartsTip, "class": "esriCTEventsImg" }, null);
            ...
            */
            ...
            // Function to 调整图表大小 on click.
            this.own(on(this.adjustSizeSpanDojoPoint, a11yclick, lang.hitch(this, function () {
                var ele = this.adjustSizeSpanDojoPoint;
                if(ele.innerHTML == "放大"){
                    this.applicationHeaderActivityContainer.style.width = "800px";
                    this.adjustSizeSpanDojoPoint.innerHTML = "缩小";
                    this.showChart();//重新生成图表
                }else{
                    this.applicationHeaderActivityContainer.style.width = "";
                    this.adjustSizeSpanDojoPoint.innerHTML = "放大";
                    this.showChart();//重新生成图表
                }
            })));
        ...
        */
        ...
        //
        showChart:function(){
            url = dojoChartConfig.url;
            countByCategoryFieldName = dojoChartConfig.FieldName;
            var chartType = dojoChartConfig.chartType;
            //url = "http://services.arcgis.com/AegVO92BkdKrxZ0V/arcgis/rest/services/PublicStationMapService/FeatureServer/0";
            //countByCategoryFieldName = "statusCode";
            ...
            queryTask.execute(query, function(results){
                var thisParam = dojo.thisParam;
                var resultCount = results.features.length;
                CategotykeyPairs = new HashMap();
                for (var i = 0; i < resultCount; i++) {
                    var fieldValue =results.features[i].attributes[countByCategoryFieldName];
                    //var fieldAlias = results.fields[i].alias;
                    if(CategotykeyPairs.containsKey(fieldValue)) {
                        var count = CategotykeyPairs.get(fieldValue);
                        count++;   
                        CategotykeyPairs.put(fieldValue,count);
                    }
                    else{
                        var count = 0;
                        CategotykeyPairs.put(fieldValue,count);
                ...
                //clearIntervalChart = setTimeout('thisParam.barChart()',1000);//调用成员函数必须得加() 区别调用 成员变量
                //clearIntervalChart = setTimeout('thisParam.barChart',1000);//这个不行。这个是去找 barChart的成员变量了。
                //clearIntervalChart = setTimeout('barChart2()',1000);
                //thisParam.barChart(true);
                thisParam.barChart();
            });
            //queryTask.execute(query, this.categoryChartGetCount);
        },

        //barChart:function(flag){
        barChart:function(){
            var a = 1;
            bchart = new Highcharts.Chart({
                chart: {
                    ...
                    events: {
                        load: function () {
                            ...
                            var data = [];
                            var x =[];
                            x  = CategotykeyPairs.keySet();
                            for (var i = 0; i < x.length; i++) {
                                data.push([x[i],CategotykeyPairs.get(x[i]) ]);
                            }
                            series.setData(data);
                            // }, 5);
                        }
                ...
                title: {
                    text: '<b>北京市新能源汽车充电桩</b>'
                },
                xAxis: {
                    categories: CategotykeyPairs.keySet(),
                    labels: {
                        style: {
                            fontSize: '13px',
                        ...
                        formatter:function(){
                            if( typeof this.value== "string") {
                                return this.value.substring(0, 5) + "<br/>" + this.value.substring(5, 10) + "<br/>" + this.value.substring(10, 15) + "<br/>" + this.value.substring(15, 20) + "<br/>";
                            }
                        }
                    }
                },
